...
void NetworkManager::run_async(int num_workers) {
    ...
    for (int i = 0; i < num_workers; ++i) {
        auto worker_ptr = new Worker(&zmq_context, nodeStore); // Створюємо вказівник
...
void NetworkManager::triggerSensor(NodeId sensorId) {
    Printer::print_safe("[Manager] Тригер сенсора " + std::to_string(sensorId.level) + "." + std::to_string(sensorId.index));
    ...
    try {
        ...
        if (result) {
            Printer::print_safe("[Manager] Повідомлення тригера успішно надіслано (" + std::to_string(result.value()) + " байт)."); // <-- Успіх
        } else {
            Printer::print_safe("[Manager] Помилка надсилання повідомлення тригера (send повернув false)."); // <-- Помилка?
        }
    } catch (const zmq::error_t& e) {
        Printer::print_safe("[Manager] ZMQ Помилка при надсиланні тригера: " + std::string(e.what())); // <-- Помилка ZMQ?
    }
}
...
void NetworkManager::createLevel(const int level) {
    ...
    Printer::print_safe("[Manager] Створено генералізований вузол рівня " + std::to_string(level) + " і зв'язано з finalNode");
}
...
void NetworkManager::setupNodeCallback(const std::shared_ptr<Node> &node) {
    ...
    node->OnRecalculated =
        [sender = sender_ptr](const MessageSend &message) {
            ...
            Printer::print_safe("[Вузол " + message.senderId.to_string() +"]: - відправлено дані " + std::to_string(message.data));
    };
}